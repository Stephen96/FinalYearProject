...
for col in predictors:# minus stats of fighter 2 from those of fighter 1
    new_fights[col] = new_fights.apply(
        lambda row: new_fighters.loc[row["fighter1"], col] - new_fighters.loc[row["fighter2"], col], axis=1)


new_fights.drop(["fighter1", "fighter2"], axis=1, inplace=True)#drop fighter name columns
...
model = Sequential([Dense(32, input_dim=X_train.shape[1], activation='relu', kernel_regularizer=regularizers.l2(0.01)), #5 layered model with 1 input layer
